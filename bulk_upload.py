...
def connect() -> girder_client.GirderClient:
    ...
    girder_client.GirderClient
        Authenticated Girder client object.

    Notes
    -----
    The returned object, `gc`, is used to make API calls such as:
        gc.get(...)
        gc.post(...)
        gc.listItem(...)
        gc.listFolder(...)
    ...
    # Create a Girder client pointing at the WASABI API.
    gc = girder_client.GirderClient(apiUrl=API_URL)

    # Authenticate using the API key stored in the environment variable.
    gc.authenticate(apiKey=API_KEY)

    # Return the authenticated client so other functions can use it.
    return gc


# ---------------------------------------------------------------------
# WASABI/Girder folder traversal
# ---------------------------------------------------------------------
...
def iter_items_recursive(gc: girder_client.GirderClient, folder_id: str):
    ...
    ------
    dict
        Girder item dictionaries. In this context, each item should usually
        correspond to one WSI.

    Explanation
    -----------
    A WASABI/Girder folder may contain:
        - WSI items directly inside it
        - subfolders
        - WSI items inside those subfolders

    This function walks through all levels of the folder tree.
    """
    # First yield all items directly inside this folder.
    for item in gc.listItem(folder_id):
        yield item

    # Then look for subfolders inside this folder.
    for subfolder in gc.listFolder(folder_id):
        # For each subfolder, recursively yield its items too.
